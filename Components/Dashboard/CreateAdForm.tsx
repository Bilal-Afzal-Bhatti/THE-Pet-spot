...
export default function CreateAdForm({ onSubmit, isSubmitting }: CreateAdFormProps) {
  ...
  const [aiMismatchData, setAiMismatchData] = useState<{
    expectedCategory: string;
    detectedCategory: string;
    confidence: number;
  } | null>(null);

  // Actual submission logic, pulled out so both the normal path and the
  // "keep this photo anyway" path (from the modal) can call it.
  const submitAd = async () => {
    ...
    Object.entries(formData).forEach(([key, value]) => {
      if (key === 'images') {
        ...
        // Skip separate province and city, we combine them into location below
      } else {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (
      (formData.category === 'dog' || formData.category === 'cat') &&
      formData.images.length > 0
    ) {
      setIsValidatingAI(true);
      try {
        const validationFormData = new FormData();
        validationFormData.append("file", formData.images[0]);

        const aiResponse = await fetch(
          `http://127.0.0.1:8000/predict?expected_category=${formData.category}`,
          {
            method: "POST",
        ...
        if (aiResponse.ok) {
          ...
          if (aiResult.mismatch) {
            ...
            setAiMismatchData({
              expectedCategory: formData.category,
              detectedCategory: aiResult.predicted_pet,
              confidence: aiResult.confidence,
            });
            setAiModalOpen(true);
            return; // Stops submission — user must resolve the mismatch first
          }
        }
      } catch (err) {
        console.warn("AI server unreachable, skipping validation...", err);
      } finally {
        setIsValidatingAI(false);
      }
    }

    // 2 & 3. Build payload and submit
    await submitAd();
  };

  const handleChooseDifferentPhoto = () => {
    setAiModalOpen(false);
    // Clear the current image(s) so the user is prompted to re-upload
  ...
  const handleKeepPhotoAnyway = async () => {
    // User acknowledged the AI warning and wants to submit as-is
    setAiModalOpen(false);
    await submitAd();
  };
    ...
    <div className="space-y-6">
      {aiValidationError && (
        <div className="p-4 bg-red-50 border-l-4 border-red-500 text-red-700 rounded-r-xl text-sm">
          <p className="font-bold">AI Verification Warning</p>
          <p>{aiValidationError}</p>
        </div>
      )}
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-between pt-6">
          ...
          {currentStep < totalSteps ? (
            ...
            <button
              type="submit"
              disabled={isSubmitting || isValidatingAI || !validateStep(currentStep)}
              ...
              style={!validateStep(currentStep) || isSubmitting || isValidatingAI ? {} : { background: 'var(--gradient-hero)' }}
            >
              {isValidatingAI ? (
                <>
      ...
      {aiMismatchData && (
        <AIVerificationModal
          isOpen={aiModalOpen}
          expectedCategory={aiMismatchData.expectedCategory}
          detectedCategory={aiMismatchData.detectedCategory}
          confidence={aiMismatchData.confidence}
          onChoosePhoto={handleChooseDifferentPhoto}
          onClose={handleKeepPhotoAnyway}
        />
      )}
    </div>
  );
